...
Dim c As Long
    ...
    Do While c <= CheckNum
        i = i + 1
        If LenB(Var_Get(App.Path & "\Dev\GrhRaw.txt", "START", i)) = 0 Then
            GrhValues(c) = i
            c = c + 1
        End If
    Loop
    ...
    Do While c <= CheckNum
        i = i + 1
        If Not FileExist(App.Path & "\Graphics\" & i & ".PNG", vbNormal) Then
            GrhFiles(c) = i
            c = c + 1
        End If
    Loop
...
On Error GoTo ErrOut
    ...
    If LenB(Dir$(File, FileType)) <> 0 Then FileExist = True
